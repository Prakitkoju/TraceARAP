(function ($) {
  ...
  $('#dramt').on('input', function () {
    $('#cramt').val("0");
  });

  $('#cramt').on('input', function () {
    $('#dramt').val("0");
  });
  ...
  function openAddModal(e) {
    ...
    transService.getMaxId()
      .then((maxid) => {
        ...
        // console.log(maxid['0']===null)
        // console.log(maxid['0'])
        if (maxtransid === null) {
          maxtransid = 1
        }
        const formData = $modal.find('form').serialize() + '&user_id=' + user_idvar + '&doc_type=OPB' + '&trans_id=' + maxtransid;
