    ...
    transService
      ...
      .then(function (result) {
        ...
        for (grpled in result) {
          if ((result[grpled]['is_system'] === "Y" && result[grpled]['cashbank'] === "V")) {
            // ledclients = result[grpled]['grpledger_id']
            $ledtag.append(
              $('<option>').val(result[grpled]['grpledger_id']).text(result[grpled]['name'])
            )
          } else if ((result[grpled]['is_system'] === "Y" && result[grpled]['cashbank'] === "P")) {
            $('#cashbanksel').append(
              $('<option>').val(result[grpled]['grpledger_id']).text(result[grpled]['name'])
            )
          }
  ...
  function openAddModal(e) {
    ...
    transService.getMaxId()
      .then((maxid) => {
        ...
        const formData = $modal.find('form').serialize() + '&user_id=' + user_idvar + '&doc_type=PUR' + '&cr_amt=0' + '&trans_id=' + maxtransid;
